...
async function checkTables() {
  ...
  for (const result of results) {
    ...
    if (result.status === '✅') {
      ...
      console.log(`  └─ Erro: ${result.error.substring(0, 60)}...`);
    }
  }
...
async function checkData() {
  ...
  const queries = [
    ...
    { name: 'Agentes IA', table: 'agentes_ia' },
  ];
  ...
  for (const { name, table } of queries) {
    ...
    if (error) {
      ...
      console.log(`✅ ${name}: ${data?.length || 0} registro(s)`);
...
async function generateReport() {
  ...
  if (scorePercentage === 100 && migrationsOk) {
    ...
    if (!migrationsOk) {
      ...
      console.log('      cd supabase');
      console.log('      supabase link --project-ref yfxgncbopvnsltjqetxw');
      console.log('      supabase db push');
    }
    ...
    if (tableResults.totalRecords === 0) {
      ...
      console.log('      OU execute: node apply-test-data.js');
    }
    console.log();
  } else {
    console.log('❌ SISTEMA NÃO CONFIGURADO');
    console.log('\n💡 Ações necessárias:');
    console.log('   1. Linkar projeto Supabase:');
    console.log('      cd supabase');
    console.log('      supabase link --project-ref yfxgncbopvnsltjqetxw');
    console.log('   2. Aplicar todas as migrations:');
    console.log('      supabase db push');
    console.log('   3. Verificar conexão com Supabase');
    console.log('   4. Confirmar credenciais no .env\n');
  }
