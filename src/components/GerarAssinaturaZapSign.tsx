...
export const GerarAssinaturaZapSign = ({ contrato, onSuccess }: GerarAssinaturaZapSignProps) => {
  ...
  const { data: lead } = useQuery({
    queryKey: ['lead', contrato.lead_id],
    queryFn: async () => {
      if (!contrato.lead_id) return null;
    ...
    enabled: !!contrato.lead_id
  });

  // Preencher automaticamente os dados do lead
  React.useEffect(() => {
    if (lead) {
      setEmail(lead.email || '');
  ...
  const handleGerarLink = async () => {
    if (!email) {
      toast.error('E-mail é obrigatório para gerar o link de assinatura');
      return;
    }
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            <p className="text-sm text-blue-800">
              <strong>Área:</strong> {contrato.area_juridica}
            </p>
          </div>
